...
class View:
    ...
    def menu_notas(ventana, usuario_id, nombre, apellidos):
        ...
        btn_cambiar = Button(ventana, text="3.- Cambiar", command=lambda: View.cambiar_notas(ventana, nom_user, ape_user))
        btn_cambiar.pack(pady=15)

        btn_eliminar = Button(ventana, text="4.- Eliminar", command=lambda: View.eliminar_nota(ventana, nom_user, ape_user))
    ...
    @staticmethod
    def mostrar_notas(ventana,id_user, nom_user, ape_user):
        View.borrar_pantalla(ventana)
        ...
        if len(registros) < 0:
            ...
            columns = ("id_nota", "titulo", "descripcion", "fecha")
    
    
        tree = ttk.Treeview(ventana, columns=columns, show="headings", height=10)
    
        tree.heading("id_nota", text="ID de la nota")
        tree.heading("titulo", text="Título")
        tree.heading("descripcion", text="Descripción")
        tree.heading("fecha", text="Fecha")

        tree.column("id_nota", width=50, anchor="center")
    ...
    #En esta parte del codigo se llenan los registros
        for f in registros:
       
            tree.insert("", END, values=(f[0], f[2], f[3], f[-1]))
    ...
    @staticmethod
    def cambiar_notas(ventana, nom_user, ape_user):
        View.borrar_pantalla(ventana)

        lbl_titulo = Label(ventana, text=f"{nom_user} {ape_user} vamos a modificar una nota")
        lbl_titulo.pack(pady=10)
        ...
        btn_guardar = Button(ventana, text="Guardar", command=lambda: controlador_1.Controlador.cambiar_nota(entry_id.get(), entry_nuevo.get(), entry_desc.get()))
        btn_guardar.pack(pady=15)
    ...
    @staticmethod
    def eliminar_nota(ventana, nom_user, ape_user):
        View.borrar_pantalla(ventana)

        lbl_titulo = Label(ventana, text=f"{nom_user} {ape_user} vamos a eliminar una Nota")
        lbl_titulo.pack(pady=10)
        ...
        btn_eliminar = Button(ventana, text="Eliminar", command=lambda: controlador_1.Controlador.eliminar_nota(entry_id.get()))
        btn_eliminar.pack(pady=15)
